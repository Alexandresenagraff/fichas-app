...
export default function Conferencia() {
  ...
  async function carregarFichas() {
    try {
      ...
      lista.sort((a, b) => {
        if (a.conferencia !== b.conferencia)
          return a.conferencia ? 1 : -1;
  ...
  function estaVencido(ficha: any): boolean {
    if (!ficha.entrega || ficha.conferencia) return false;
  ...
  const pendentes = fichasFiltradas.filter((f) => !f.conferencia);
  const concluidas = fichasFiltradas.filter((f) => f.conferencia);
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="grid grid-cols-2 gap-3 mb-5">
          ...
          <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-3 text-center">
            ...
            <p className="text-xs text-zinc-400">CONFERIDOS</p>
          </div>
        </div>
