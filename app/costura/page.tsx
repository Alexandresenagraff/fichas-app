    ...
    try {
      ...
      if (campo === "costura") {
        if (!valorAtual) {
          ...
          atualizacao.costuraData = `${dia}/${mes}/${ano} ${horas}:${minutos}`;
        } else {
          atualizacao.costuraData = "";
        }
      }
      ...
      setFichas((prev) =>
        prev.map((item) =>
          item.id === id ? { ...item, [campo]: !valorAtual, ...(campo === "costura" ? { costuraData: atualizacao.costuraData } : {}) } : item
        )
      );
    } catch (error) {
      console.log(error);
